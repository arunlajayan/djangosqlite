...
def countries_list(request):
    if request.method == "GET":
        ...
        countries.data = JSONParser().parse(request)
...
def countries_detail(request, pk):
    try:
        ...
        return JsonResponse(
            {"message": "The country does not exist"}, status=status.HTTP_404_NOT_FOUND
        )

    if request.method == "GET":
        countries_serializer = CountriesSerializer(countries)
        return JsonRespose(countries_serializer.data)
        ...
        return JsonResponse(
            countries_serializer.errors, status=status.HTTP_404_NOT_FOUND
        )
    elif request.method == "DELETE":
        countries.delete()
        return JsonResponse(
            {"message": "country was deleted suceefully"},
